...
uv run context-router init --project-root /path/to/repo
uv run context-router index --project-root /path/to/repo

# Run the 20-task benchmark suite
uv run context-router benchmark run --project-root /path/to/repo

# Print the Markdown report
uv run context-router benchmark report --project-root /path/to/repo
```
...
- **Hit rate**: fraction of task `expected_symbols` anchor keywords that appear in selected item titles.
- **Random baseline**: same item count sampled uniformly from all indexed symbols.
- **Rank quality**: fraction of selected items with confidence ≥ 0.70.
